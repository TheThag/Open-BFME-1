...
// readable body of ?getDynamicLODEnabled@OptionPreferences@@QAE_NXZ: Code/GameEngine/Source/GameClient/GUI/GUICallbacks/Menus/OptionsMenu.cpp
// Lift the OptionPreferences::getDynamicLODEnabled naked dump to clean C++.
//
...
// "m_data ? m_data+8 : empty", and the default reads TheWritableGlobalData+0x58.
//
...
bool OptionPreferences::getDynamicLODEnabled(void)
{
	PreferenceNode *it;
	{
		AsciiString key("DynamicLOD");
		it = m_prefs.find(key);
	}

	if (it == m_prefs.end())
		return TheWritableGlobalData->m_dynamicLODEnabled;
