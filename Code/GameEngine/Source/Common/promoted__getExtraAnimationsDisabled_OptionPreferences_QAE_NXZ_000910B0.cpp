...
// readable body of ?getExtraAnimationsDisabled@OptionPreferences@@QAE_NXZ: Code/GameEngine/Source/GameClient/GUI/GUICallbacks/Menus/OptionsMenu.cpp
// Lift the OptionPreferences::getExtraAnimationsDisabled naked dump to clean C++.
//
...
// "m_data ? m_data+8 : empty", and the default reads TheWritableGlobalData+0x1C.
//
// _strcmpi goes through the IAT so the comparison is a call; setne is the 8-bit != 0 that the if-form produces.
//
...
bool OptionPreferences::getExtraAnimationsDisabled(void)
{
	PreferenceNode *it;
	{
		AsciiString key("ExtraAnimations");
		it = m_prefs.find(key);
	}

	if (it == m_prefs.end())
		return TheWritableGlobalData->m_extraAnimationsDisabled;
