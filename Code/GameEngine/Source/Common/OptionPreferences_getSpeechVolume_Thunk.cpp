...
// readable body of ?getSpeechVolume@OptionPreferences@@QAEMXZ: Code/GameEngine/Source/GameClient/GUI/GUICallbacks/Menus/OptionsMenu.cpp
// Lift the OptionPreferences::getSpeechVolume naked dump to clean C++.
//
...
// vtable +0x120 and scales the voice volume it returns by 100, so the
...
// "m_data ? m_data+8 : empty", and the default volume sits at settings+0x84.
...
public:
	...
	float m_defaultVoiceVolume;				///< retail this+0x84
};
...
float OptionPreferences::getSpeechVolume(void)
{
	PreferenceNode *it;
	{
		AsciiString key("VoiceVolume");
		it = m_prefs.find(key);
	}

	if (it == m_prefs.end())
		return TheAudio->getAudioSettings()->m_defaultVoiceVolume * 100.0f;
