...
// stlport
// Open-BFME: AICommandInterface::aiGuardArea, retail 0x00154890, 214 bytes.
//
// One of the reference's inline command builders, out of line here: build the
// parameter block, put the polygon in the polygon slot and the guard mode in the misc integer, then
// aiDoCommand at vtable slot 0 -- the frame
// AICommandInterface_aiAttackObject.cpp measured.
//
// AICMD_GUARD_AREA is 0x21. BFME's command list is the reference's with the
...
//
// One more command is inserted between GUARD_OBJECT and this one: 0x20 writes
// the team slot and an integer, which no reference command in that stretch
// does, and it is left unclaimed here.
...
enum AICommandType { AICMD_GUARD_AREA = 0x21 };
