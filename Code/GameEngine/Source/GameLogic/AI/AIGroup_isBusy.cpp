...
// stlport
// Open-BFME: AIGroup::isBusy, retail 0x001512E0, 73 bytes.
//
// The reference's body unchanged. getAIUpdateInterface() is a member read of
// Object+0x204, isBusy is vtable slot 99 (+0x18C) on the AI, and
// isEffectivelyDead() is inline -- bit 0 of the byte at Object+0x344.
//
// The reference's `state = ai->isBusy() && !obj->isEffectivelyDead(); if
...
public:
	...
	virtual Bool isBusy(void) const;			// vtable +0x18C
};
